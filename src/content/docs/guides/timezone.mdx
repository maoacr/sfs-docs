...
Drizzle devuelve objetos `Date` de JavaScript para las columnas `timestamp`. Un `Date` en JavaScript **siempre** representa un instante en UTC internamente (milisegundos desde epoch), pero sus métodos como `getHours()` usan la zona horaria local del sistema.
...
- [ ] ¿Estoy filtrando por rango de fechas en una query? → Usar hora local para los límites (`"2026-08-06T23:59:59"` sin Z)
