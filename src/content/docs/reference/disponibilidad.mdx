...
db.query.canchas.findMany({
  where: and(
    ...
    tipo ? eq(canchas.tipo, tipoCanchaToDb(tipo)) : undefined // "F5" → "FUTBOL_5"
  ),
  with: {
    complejo: true,                                       // nombre, dirección, coordenadas
    slots: { where: eq(slotConfigs.diaSemana, diaSemana) },
    tarifas: true,
    imagenes: { orderBy: (i, { asc }) => [asc(i.orden)] },
    reservas: { ... },                                    // activas del día consultado
  },
})
```

Las canchas sin `SlotConfig` para ese día se descartan al calcular los slots.
